    ...
    def enrich(self, ingredient_name: str) -> Dict[int, Any]:
        ...
        if cosing.found:
            if cosing.role:
                result[5] = cosing.role          # Role in formulation
            if cosing.description:
                result[6] = cosing.description   # Note (CosIng preferred)
            result[7] = "CosIng"                 # Source

        # Fallback: use SkinSafe description if CosIng has none
        if not result.get(6):
            ss = scrape_skinsafe(ingredient_name)
            if ss.description:
                result[6] = ss.description
                # Append SkinSafe to source if not already set
                existing_source = result.get(7, "")
                result[7] = f"{existing_source}, SkinSafe".lstrip(", ") if existing_source else "SkinSafe"
